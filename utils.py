...
def apply_speed_factor_sox_external(
    audio_tensor: torch.Tensor,
    sample_rate: int,
    speed_factor: float,
    sox_path: str = "sox",
    ...
    if speed_factor == 1.0:
        return audio_tensor, sample_rate
    if speed_factor <= 0:
        logger.warning(f"Invalid speed_factor {speed_factor}. Must be positive.")
        return audio_tensor, sample_rate
    ...
    if audio_tensor.ndim == 2 and audio_tensor.shape[0] == 1:
        ...
        logger.warning(
            f"apply_speed_factor_sox_external received multi-channel audio (shape {audio_tensor.shape}). Using first channel only."
        )
    ...
    try:
        ...
        if out_audio.size(0) == 1:
            out_audio = out_audio.squeeze(0)

        return out_audio.to(audio_tensor.device), out_sr

    except Exception as e:
        logger.error(f"External sox tempo failed: {e}", exc_info=True)
        return audio_tensor, sample_rate
